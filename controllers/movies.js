...
function createMovies(req, res, next) {
  Movie.create({ ...req.body, owner: req.user._id })
    .then((movie) => res.status(CREATE).send({"message": `${movie.nameRU} успешно добавлен в Сохранённые фильмы`}))
    .catch((err) => {
  ...
  Movie.findById(req.params.movieId)
    .then((movie) => {
      ...
      if (movie.owner.valueOf() !== req.user._id)
        throw new FORBIDDEN_ERROR("У вас нет прав удалять чужие фильмы");
      Movie.findByIdAndRemove(req.params.movieId)
        .then((movie) => {
          res.status(SUCCESS).send({"message":`${movie.nameRU} успешно удалён`});
        })
        .catch(next);
    })
    .catch(next);
